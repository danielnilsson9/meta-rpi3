...
do_deploy() {
    ...
    if [ -n "${DISABLE_RAINBOW_SPLASH}" ]; then
        sed -i '/#disable_splash=/ c\disable_splash=${DISABLE_RAINBOW_SPLASH}' ${DEPLOYDIR}/bcm2835-bootfiles/config.txt
    fi
    ...
    if [ -n "${FRAMEBUFFER_DEPTH}" ]; then
        sed -i '/#framebuffer_depth=/ c\framebuffer_depth=${FRAMEBUFFER_DEPTH}' ${DEPLOYDIR}/bcm2835-bootfiles/config.txt
    fi
